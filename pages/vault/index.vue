    ...
    <v-row justify="center">
          ...
          >
            <template v-slot:header.supplements="{ header }">
              ...
              <v-tooltip bottom>
                ...
                <span
                  >Indicate the version, the homebrew was originaly designed
                  for.</span
                >
              </v-tooltip>
            </template>
            ...
            <template v-slot:item.supplements="{ item }">
              ...
              <v-chip v-else tags x-small label>
                <span>{{ item.supplements }}</span>
              </v-chip>
            </template>
            ...
            <template v-slot:expanded-item="{ headers, item }">
              <td :colspan="headers.length">
                <v-row>
                  ...
                  <v-col
                    v-if="item.image && item.image.fields.file.url && false"
                    :cols="12"
                    :md="3"
                  >
                    ...
                    <ul>
                      <li v-for="parts in item.contentTags" :key="parts">
                        <nuxt-link
                          ...
                          "
                          :to="`/library/${textToKebab(parts)}?filter-source=${
                            item.sourceKey
                          }`"
                        >
                          {{ parts }}
                ...
                <v-card-actions>
                  <v-btn
                    ...
                    target="_blank"
                    @click="trackEvent(item.documentUrl)"
                  >
                    View the document <v-icon right dark> launch </v-icon>
                  </v-btn>
      ...
      <v-col :cols="12">
        <v-card>
          <v-card-text>
            ...
            <p>
              In desperate need of an option to extend your Wrath & Glory
              Campaign? Searching for <em>that</em> particular homebrew to
              improve your setting? The <strong>Vault</strong> has you covered.
              This is a <strong>curated list of homebrews</strong> from fans,
              found in the internet. I credit the author and link to their
              community pages, as good as I could, if I find them either in the
              document found or on their respective page. If want to add, remove
              or change your homebrew content or if you want to propose changes
              regarding links, you can mail me to
              <a href="mailto:docsofdoom+vault@gmail.com?subject=Vault Request"
                >docsofdoom+vault(at)gmail.com</a
              >.
            </p>
          </v-card-text>
        </v-card>
      ...
      <v-col :cols="12">
        <v-expansion-panels accordion>
          <v-expansion-panel v-for="item in faqItems" :key="item.q">
            <v-expansion-panel-header>{{ item.q }}</v-expansion-panel-header>
            <v-expansion-panel-content>
              <p class="body-2" v-html="item.a" />
            </v-expansion-panel-content>
          </v-expansion-panel>
        </v-expansion-panels>
      </v-col>
...
export default {
  ...
  head() {
    const itemSchemaArray = this.vaultItems.map((item) => {
      return {
        ...
        thumbnailUrl: item.image
          ? `https://www.doctors-of-doom.com${item.image.fields.file.url}`
          : null,
        description: item.abstract,
        keywords: item.keywordTags
          ? [...item.keywordTags, "Wrath & Glory"].join(",")
          : "Wrath & Glory",
      };
    });
    ...
    const image =
      "https://www.doctors-of-doom.com/img/artwork_vault_bright.jpg";
    ...
    return {
      ...
      script: [
        {
          innerHTML: JSON.stringify(itemSchemaArray),
    ...
    searchResults() {
      ...
      if (this.contentFilter.length > 0) {
        filteredResults = filteredResults.filter((item) => {
          ...
          if (item.contentTags) contentOptions.push(...item.contentTags);
          if (item.keywordTags) contentOptions.push(...item.keywordTags);
          return [...new Set(contentOptions)].some((c) =>
            this.contentFilter.includes(c)
          );
        });
      }
